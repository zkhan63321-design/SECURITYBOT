...
// Detects mass role deletions (nuke attempts)
      ...
      if (data.count >= THRESHOLD) {
        ...
        if (member) {
          ...
          await guild.members.ban(executor.id, {
            reason: '🔒 Anti-Nuke: Mass role deletion detected'
          }).catch(() => {});
        }

        logAction(guild, `🚨 **ROLE NUKE STOPPED!** <@${executor.id}> was mass-deleting roles. **Banned + roles stripped.**`);
      }
    }
  }
};
